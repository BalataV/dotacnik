...
function Coin({ left, delay, dur }: { left: number; delay: number; dur: number }) {
...
export function Coins({ show }: { show: boolean }) {
  const coins = useMemo(
...
export function Toast({ text, ink }: { text: string | null; ink: string }) {
  const op = useRef(new Animated.Value(0)).current;
  useEffect(() => {
    if (text) {
      op.setValue(0);
