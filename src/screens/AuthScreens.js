  ...
  return (
    <ScrollView contentContainerStyle={{ flexGrow: 1, alignItems: 'center', justifyContent: 'center', padding: 28, paddingVertical: 40 }}>
      <View style={{ position: 'relative', marginBottom: 14 }}>
        ...
        <View style={{ backgroundColor: '#fff', borderWidth: 3, borderColor: c.ink, borderRadius: 18, paddingVertical: 10, paddingHorizontal: 20 }}>
          <Text style={{ fontFamily: FONTS.display600, fontSize: 22, color: '#15233B' }}>Čau lidi!</Text>
        </View>
      </View>
      <Mascot size={130} float />
      <Text style={{ fontFamily: FONTS.display700, fontSize: 38, color: c.onbg, marginTop: 14, marginBottom: 6, letterSpacing: -1 }}>Čapí Dluh</Text>
...
function BackButton({ onPress, label = '‹ Zpět' }) {
  const c = useColors();
  return (
    <Text onPress={onPress} style={{ color: c.onbg, fontFamily: FONTS.body800, fontSize: 15, marginBottom: 22 }}>{label}</Text>
  );
}
...
export function RegisterEmail() {
  ...
  return (
    <ScrollView contentContainerStyle={{ flexGrow: 1, padding: 28 }}>
      ...
      <Label>E-mail</Label>
      <Field value={state.regEmail} onChangeText={(t) => actions.patch({ regEmail: t })} placeholder="vas@email.cz" keyboardType="email-address" autoCapitalize="none" style={{ marginBottom: 14 }} />
      <Label>Heslo</Label>
      <Field value={state.regPassword} onChangeText={(t) => actions.patch({ regPassword: t })} placeholder="min. 6 znaků" secureTextEntry style={{ marginBottom: 26 }} />
      ...
      <Text onPress={() => actions.navigate('login')} style={{ textAlign: 'center', fontFamily: FONTS.body700, fontSize: 14, color: c.accent, marginTop: 20 }}>
...
export function Login() {
  ...
  return (
    <ScrollView contentContainerStyle={{ flexGrow: 1, padding: 28 }}>
      ...
      <Label>E-mail</Label>
      <Field value={state.loginEmail} onChangeText={(t) => actions.patch({ loginEmail: t })} placeholder="vas@email.cz" keyboardType="email-address" autoCapitalize="none" style={{ marginBottom: 14 }} />
      <Label>Heslo</Label>
      <Field value={state.loginPassword} onChangeText={(t) => actions.patch({ loginPassword: t })} placeholder="••••••••" secureTextEntry style={{ marginBottom: 26 }} />
      ...
      <Text onPress={() => actions.navigate('register_email')} style={{ textAlign: 'center', fontFamily: FONTS.body700, fontSize: 14, color: c.accent, marginTop: 20 }}>
