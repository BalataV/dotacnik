...
export default function CreateGroup() {
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 24 }}>
      <Text onPress={() => actions.navigate('overview')} style={{ color: c.onbg, fontFamily: FONTS.body800, fontSize: 15, marginBottom: 14 }}>‹ Zpět</Text>
      ...
      <Field value={state.newGroupName} onChangeText={(t) => actions.patch({ newGroupName: t })} placeholder="Chata, výlet, pivo…" style={{ marginBottom: 18 }} />
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 7, marginBottom: 12, minHeight: 38 }}>
        {state.newGroupMembers.map((m) => (
          <View key={m} style={{ flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: c.accent, borderWidth: 2, borderColor: c.ink, borderRadius: 20, paddingVertical: 6, paddingHorizontal: 11 }}>
            ...
            {m !== 'Já' && (
              <Text onPress={() => actions.removeMember(m)} style={{ color: '#fff', fontSize: 14, paddingHorizontal: 3 }}>×</Text>
            )}
          </View>
        ))}
      </View>
      ...
      <View style={{ flexDirection: 'row', gap: 8, marginBottom: 28 }}>
        <Field
          ...
          style={{ flex: 1 }}
        />
        <Pushable onPress={actions.addMember} offset={3} radius={13}>
