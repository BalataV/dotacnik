...
export default function ExpenseDetail() {
  ...
  const list = (state.expenses[state.selectedGroup] || []);
  ...
  const perPerson = Math.round(e.amount / e.parts.length);
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 24 }}>
      ...
      <Text style={{ fontFamily: FONTS.display700, fontSize: 34, color: c.onbg, marginBottom: 16 }}>{e.amount} Kč</Text>

      {/* Foto účtenky */}
      {e.photo && (
        <View style={{ position: 'relative', marginBottom: 16 }}>
          <View style={{ position: 'absolute', top: 4, left: 4, right: -4, bottom: -4, backgroundColor: c.ink, borderRadius: 16 }} />
          <Image source={{ uri: e.photo }} style={{ width: '100%', height: 240, borderRadius: 16, borderWidth: 3, borderColor: c.ink }} resizeMode="cover" />
        </View>
      )}
      ...
      <View style={{ position: 'relative', marginBottom: 20 }}>
        ...
        <View style={{ backgroundColor: c.card, borderWidth: 3, borderColor: c.ink, borderRadius: 14, overflow: 'hidden' }}>
          <Text style={{ fontFamily: FONTS.body800, fontSize: 11, color: c.muted, textTransform: 'uppercase', letterSpacing: 0.5, padding: 12, paddingBottom: 8 }}>Rozděleno mezi · {perPerson} Kč/os</Text>
          {e.parts.map((p, idx) => (
            <View key={p} style={{ flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 10, paddingHorizontal: 12, borderTopWidth: 2, borderTopColor: 'rgba(0,0,0,0.07)' }}>
              ...
              <Text style={{ fontFamily: FONTS.display700, fontSize: 14, color: c.ink }}>{perPerson} Kč</Text>
            </View>
          ))}
        </View>
      </View>
