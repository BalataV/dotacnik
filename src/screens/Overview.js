...
import { netFor, initial, totalOwe, totalOwed } from '../logic';
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 24 }}>
      <View style={{ flexDirection: 'row', gap: 10, marginBottom: 18 }}>
        <StatCard label="Dlužíš" value={owe + ' Kč'} color={c.bad} />
        <StatCard label="Dostaneš" value={owed + ' Kč'} color={c.good} />
      </View>
      ...
      {state.groups.length === 0 && (
        <Pushable radius={18}>
          <View style={{ backgroundColor: c.card, borderWidth: 3, borderColor: c.ink, borderRadius: 18, paddingVertical: 30, paddingHorizontal: 20, alignItems: 'center' }}>
            <Text style={{ fontFamily: FONTS.display700, fontSize: 19, color: c.ink, textAlign: 'center' }}>Zatím žádná skupina</Text>
            <Text style={{ fontFamily: FONTS.body700, fontSize: 14, color: c.muted, marginTop: 6, textAlign: 'center', lineHeight: 20 }}>
              Pozvi kámoše do Falťasova deníčku
            </Text>
          </View>
        </Pushable>
      )}
      ...
      {state.groups.map((g) => {
        const net = netFor(g, state.expenses[g.id], state.payments[g.id])['Já'] || 0;
        let netText, netColor;
        if (net < 0) { netText = 'Dlužíš ' + -net + ' Kč'; netColor = c.bad; }
        else if (net > 0) { netText = 'Dostaneš ' + net + ' Kč'; netColor = c.good; }
        else { netText = 'Vyrovnáno'; netColor = c.muted; }
        return (
