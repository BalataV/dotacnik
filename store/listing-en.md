# Google Play – listing (English)

## App name (max 30 chars)
```
Dotačník – Split Bills
```

## Short description (max 80 chars)
```
Split shared expenses with friends. Who owes whom? The app does the math.
```

## Full description (max 4000 chars)
```
Dotačník is a playful expense-splitting app for groups – friends, roommates, trips, and holidays. Who paid for the beer, who for the gas, and who got away with nothing? The app does the math for you, with a sense of humor and a mascot who "didn't read it but signed it anyway."

🧾 LOG AN EXPENSE IN SECONDS
Add what was spent, who paid, and who it's for. Attach a receipt photo if you like.

➗ FAIR SPLITTING
Split equally, by ratio (eat more, pay more), or by exact amounts. The app makes sure it all adds up to the last crown.

💸 WHO OWES WHOM
Smart settlement minimizes the number of payments – one transfer instead of ten. Settle a debt with a single tap.

🗂️ CATEGORIES & STATS
Sort expenses (food, transport, fun…) and check the parody "audit report" with a leaderboard of the night's top spenders.

🌍 MULTIPLE CURRENCIES
CZK, EUR and USD together, with an approximate conversion using live exchange rates.

👥 SHARE WITH YOUR GROUP
Invite friends by link or code. Everything syncs in real time.

🌙 LIGHT & DARK MODE
Plus an adjustable content size for comfortable reading.

🔔 NOTIFICATIONS
Someone added an expense or settled a debt? You'll know.

Dotačník is an independent satirical app. The mascot and quips are caricature, humor and hyperbole. The app is not affiliated with, operated by, or endorsed by any political party, person, or institution, and makes no factual claims about anyone.

Download, split, and let the app keep score.
```

## Category
- App category: **Finance** (alternatively Tools)
